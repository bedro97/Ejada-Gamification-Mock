...
export const UserDetails = ({ user, onBack }: UserDetailsProps) => {
  const { data: pointsHistory = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/users/${user.id}/points-history`);
      return res.json();
    },
    enabled: !!user.id,
  });
  ...
  const { data: userLevel = "" } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/users/${user.id}/level`);
      return res.text();
    },
    enabled: !!user.id,
  });
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{user.badges ?? 0}</p>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>All points awarded to this user</CardDescription>
            </CardHeader>
            <CardContent>
